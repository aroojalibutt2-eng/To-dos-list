    ...
    <div style={{ fontFamily:"'Inter','Helvetica Neue',Arial,sans-serif", display:"grid", gridTemplateColumns:"230px 1fr", minHeight:"100vh" }}>
      ...
      <main className="main-pad" style={{ background:C.pageBg, padding:"34px 30px 80px", overflowY:"auto", minHeight:"100vh" }}>
        ...
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:10 }}>
          <span style={{ fontSize:11, color:C.text3, fontWeight:700, textTransform:"uppercase", letterSpacing:0.7 }}>
            {list.length} task{list.length!==1?"s":""}
            {filter==="all"&&!q&&<span style={{ color:C.text3, fontWeight:400 }}> — drag to reorder</span>}
          </span>
          {doneN>0 && (
            <button onClick={clear} style={{
              fontSize:11.5, color:"#B83232", background:"transparent", border:"none",
              cursor:"pointer", fontFamily:"inherit",
              textDecoration:"underline", textDecorationColor:"#E8BCBC",
            }}>Clear {doneN} completed</button>
          )}
        </div>
        ...
        {list.length === 0 ? (
          <div style={{
            textAlign:"center", padding:"4rem 0", color:C.text3,
            background:C.card, border:`1px solid ${C.border}`, borderRadius:10, boxShadow:C.shadow,
          }}>
            <div style={{ fontSize:30, marginBottom:12, opacity:0.2 }}>
              {q?"○":filter==="done"?"✓":filter==="overdue"?"✓":"·"}
            </div>
            <div style={{ color:C.text2, fontWeight:500, fontSize:13.5 }}>
              {q?`No results for "${q}"` : filter==="done"?"Nothing completed yet." : filter==="overdue"?"No overdue tasks — great work." : "No tasks here."}
            </div>
            {!q && filter==="all" && (
              <div style={{ fontSize:12, color:C.text3, marginTop:6 }}>
                Click <strong style={{color:C.accent}}>New task</strong> above to get started.
              </div>
            )}
          </div>
        ) : (
          <DraggableList
            items={list}
            onReorder={newList=>{
              // merge reordered list back into todos preserving items not in list
              const ids = new Set(newList.map(t=>t.id));
              const rest = todos.filter(t=>!ids.has(t.id));
              setTodos([...newList,...rest]);
            }}
            renderItem={(t, dragProps, isDragging, isOver)=>(
              <div key={t.id} {...dragProps}>
                <TaskCard
                  t={t}
                  onToggle={toggle}
                  onDelete={del}
